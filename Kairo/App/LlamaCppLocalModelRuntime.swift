...
actor LlamaCppSession {
    ...
    func generate(
        ...
        while consumedPromptTokens < promptTokens.count {
            ...
            let now = Date()
            if now.timeIntervalSince(lastPrefillProgressAt) >= 1 || consumedPromptTokens == promptTokens.count {
...
struct LlamaCppLocalModelRuntime: LocalModelReplyCheckRuntime, LocalModelBenchmarkEngine {
    ...
    static func clearLoadedSessions() async {
        await sessionPool.removeAll()
    }
...
extension LlamaCppLocalModelRuntime: LocalModelConversationalReplyRuntime {
    func generateReply(
        ...
        await AIInferenceProgressCenter.shared.publish(AIInferenceProgressSnapshot(
            ...
            metrics: AIInferenceMetrics(stage: .preparingInput)
        ))
        ...
        if pooledSession.didLoadModel {
            await AIInferenceProgressCenter.shared.publish(AIInferenceProgressSnapshot(
                conversationID: conversationKey.conversationID,
                metrics: AIInferenceMetrics(stage: .loadingModel)
            ))
        }
        let prompt = pooledSession.isNewConversation ? initialPrompt : turnPrompt
        let preparedPrompt = Self.preparedPrompt(prompt, for: model)
        let output = try await pooledSession.session.generate(
            prompt: preparedPrompt.text,
            maxTokens: Int32(max(1, clampedParameters.maxOutputTokens)),
            resetContext: pooledSession.isNewConversation,
            addBOS: pooledSession.isNewConversation && preparedPrompt.addBOS,
            progress: { metrics in
                Task {
                    await AIInferenceProgressCenter.shared.publish(AIInferenceProgressSnapshot(
                        conversationID: conversationKey.conversationID,
        ...
        return LocalModelReplyCheckResult(
            ...
            peakMemoryMB: output.peakMemoryMB,
            measuredAt: Date(),
            notes: pooledSession.isNewConversation
                ? "Generated by resetting a warm Kairo local model session for this conversation."
...
private struct PooledLlamaCppSession: Sendable {
    ...
    var didLoadModel: Bool
}

private struct LocalModelLoadedRuntimeKey: Hashable, Sendable {
    var modelID: String
...
private actor LlamaCppSessionPool {
    ...
    private var sessions: [LocalModelLoadedRuntimeKey: StoredSession] = [:]
    private var mostRecentKeys: [LocalModelLoadedRuntimeKey] = []
    ...
    func session(
        ...
        let loadedKey = LocalModelLoadedRuntimeKey(
            modelID: key.modelID,
            modelFilePath: key.modelFilePath,
            contextSize: key.contextSize,
            maxOutputTokens: key.maxOutputTokens,
            temperature: key.temperature
        )
        if let stored = sessions[loadedKey] {
            sessions[loadedKey] = StoredSession(session: stored.session, conversationID: key.conversationID)
            touch(loadedKey)
            return PooledLlamaCppSession(
                session: stored.session,
                isNewConversation: stored.conversationID != key.conversationID,
                didLoadModel: false
            )
        }
        ...
        sessions[loadedKey] = StoredSession(session: session, conversationID: key.conversationID)
        touch(loadedKey)
        trimIfNeeded()
        return PooledLlamaCppSession(session: session, isNewConversation: true, didLoadModel: true)
    }

    private func touch(_ key: LocalModelLoadedRuntimeKey) {
