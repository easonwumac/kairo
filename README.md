...
- Agent skill catalog, manifest signature metadata, SHA-256 checksum validation, public-key trust-store verification, file-backed skill install lifecycle, and Access Skill Manager UI, so installed tools can be shown to the model and managed by the user.
