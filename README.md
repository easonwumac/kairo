...
- Structured Shortcut node runtime, executable official demo recipe runner, Settings demo recipe UI including Email Triage, Message Reply Handoff, Contact Draft, Email Draft, Request to Recipe Draft, Meeting Text to Calendar Draft, and Home Action Preview, and App Intents for asking, saving, searching, summarizing, task extraction, draft replies, reminder drafts, calendar drafts, contact drafts, email drafts, visible message handoffs, disabled internal recipe draft previews, daily briefings, and generic node-kind plus JSON input/output chaining.
