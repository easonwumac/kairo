...
- Downloadable local-model catalog entries for public GGUF models at 2B parameters or below, including Qwen3.5, Qwen2.5, SmolLM2, and TinyLlama variants, plus `LocalModelCatalogService` for the planned `kairo-models` static backend, verified downloader, install registry, selected-model settings, Settings model management UI/catalog refresh, and provider-routing scaffold. Model weights are not bundled in the app or repository.
