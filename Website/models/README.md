...
The seed catalog intentionally starts with Qwen-only Apache-2.0 entries: Qwen3.5 0.8B, Qwen3.5 2B, and Qwen2.5-VL 3B with its projector companion metadata. Larger catalogs can live in the future standalone `kairo-models` repository. Noncommercial, custom, gated, or unclear license terms should stay manual/import-only until Kairo has explicit license approval UI and bundled notices for that family.
