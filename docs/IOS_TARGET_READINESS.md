...
Current device availability was checked with `xcrun devicectl list devices` on 2026-06-04 10:37 CST, and all listed physical devices were `unavailable`. Until a signed build runs on a reachable physical iPhone or iPad, the checks below remain release-blocking and must not be replaced by simulator build output, package tests, release hygiene checks, or screenshots from `tmp/`.
