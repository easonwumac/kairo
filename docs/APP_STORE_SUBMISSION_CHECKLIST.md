...
- Privacy-label change triggers are still absent from the submitted binary: no analytics SDK, no backend account, no cloud memory sync, no crash/telemetry collection provider, and no provider-side sync beyond explicit user-configured API calls.
