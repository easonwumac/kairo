...
```text
Share Sheet / copied message / manual Shortcut input
→ Prepare Message Handoff with Kairo
→ Review recipient and body
→ Optional visible Messages handoff
```

Node contract:

- node: `prepareMessageHandoff`
- input: message body text explicitly provided by the user; optional `recipient` or `body` variables
...
```text
Share Sheet / copied phone number / manual Shortcut input
→ Prepare Phone Call Handoff with Kairo
→ Review number and label
→ Optional visible Phone handoff
```

Node contract:

- node: `preparePhoneCallHandoff`
- input: phone number text explicitly provided by the user; optional `phoneNumber` and `label` variables
- output: `phoneCallDrafts`, `proposedActions`, `fields.phoneCallHandoffCount`, `fields.phoneCallNumber`, `fields.phoneCallURL`, and `fields.phoneCallRequiresConfirmation`
- boundary: returns a visible `tel:` handoff only; Kairo does not read call history, place calls silently, or claim the call connected
...
- output: display text, typed fields, optional chain text for downstream nodes, optional memory id, memory matches, extracted task drafts, reminder drafts, calendar drafts, contact drafts, email drafts, phone call handoff drafts, and proposed actions.
...
- safety: task extraction, reminder creation, calendar/contact/email draft creation, and message/phone handoff preparation only produce drafts or visible handoffs; they do not write EventKit or Contacts data, send mail, send messages, or place calls unless a later user-confirmed system surface does so.
