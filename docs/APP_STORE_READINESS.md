...
- **Real device blocked:** `xcrun devicectl list devices` on 2026-06-04 10:23 CST listed all paired physical devices as `unavailable`, so this pass did not produce real-device sign-off.
...
Current `devicectl` check on 2026-06-04 10:23 CST found real devices listed but unavailable:
...
- [ ] 真機 smoke 尚未在這一輪重跑；2026-06-04 10:23 CST 的 `xcrun devicectl list devices` 顯示可見裝置皆為 `unavailable`，Chat / Memory / Access / Settings / Share Extension / App Intents 仍需實機簽核。
